...
class NSTM(nn.Module):
    ...
    def forward(self, input):
        theta = self.get_theta(input['data'])
        beta = self.get_beta()
        M = 1 - beta
        sh_loss = sinkhorn_loss(M, theta.T, F.softmax(input['data'], dim=-1).T, lambda_sh=self.sinkhorn_alpha)
        recon = F.softmax(torch.matmul(theta, beta), dim=-1)
        recon_loss = -(input['data'] * recon.log()).sum(axis=1)
